...
#Define method to show marks
    ...
    #         stu_id = input("Enter student ID for calculating GPA: ")
    ...
    #             for course in self.course:
    #                 np.append(credits,[self.course[course].course_credit])
    #             gpa = np.average(marks,weights=credits)
    #             print(f"GPA of Student with ID: {stu_id} is {gpa}")
    #     print (" ")
            
    def numpytest(self):
        # temp_mark = np.empty(0)
        # # temp_credit = np.empty(0)
        # temp_mark = []
        temp_credit = []
        ...
        # print(mark_real.shape)
        # print(credit_real.shape)
            # temp_credit = []
            # credits = np.append(temp_credit,self.course[course].course_credit)
        # print(marks)
        # print(credits)
        gpa = np.average(mark_real,weights=credit_real)
        print(gpa)
        # gpa = dict(sorted(self.gpa.items(),key=lambda item: item[1], reverse=True))

        # print(f'Student ID: {stu_id}, GPA: {gpa}')
        # print(" ")
